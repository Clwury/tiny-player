...
impl EmbyClient {
    ...
    pub fn mark_item_played(&self, server: &CachedServer, item_id: &str) -> Result<UserItemData> {
        self.set_played(server, item_id, true)
    }

    #[instrument(skip(self, server), fields(server = %server.endpoint.display_url(), item_id = %item_id, played))]
    pub fn set_played(
        &self,
        server: &CachedServer,
        item_id: &str,
        ...
        self.send_authenticated_json_url(server, method, url, "解析 Emby 观看状态响应失败")
    }
