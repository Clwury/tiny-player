...
pub struct VulkanDecodeDevice {
    pub(in crate::render_host) device_ref: FfmpegAvBufferRef,
    pub instance: usize,
...
impl FfmpegFrameRef {
    pub(crate) fn new_ref(frame: *const ffmpeg_ffi::AVFrame) -> Result<Self> {
        if frame.is_null() {
...
pub struct FfmpegAvBufferRef {
    pub(in crate::render_host) ptr: *mut ffmpeg_ffi::AVBufferRef,
}

impl FfmpegAvBufferRef {
    pub(crate) fn new_ref(buffer: *mut ffmpeg_ffi::AVBufferRef) -> Result<Self> {
        if buffer.is_null() {
