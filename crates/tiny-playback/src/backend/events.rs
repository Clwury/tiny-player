...
pub struct PlaybackCacheConfig {
    ...
    /// Runtime fallback supplied by the application. Explicit `cache_dir` and
    /// per-layer environment overrides take precedence. Without any directory,
    /// disk caching is unavailable; the engine never chooses an application path.
    #[serde(skip)]
...
impl PlaybackCacheConfig {
    pub(crate) fn cache_directory(&self, override_variable: &str) -> Option<PathBuf> {
        self.resolve_cache_directory(std::env::var(override_variable).ok().map(PathBuf::from))
    }

    fn resolve_cache_directory(&self, environment: Option<PathBuf>) -> Option<PathBuf> {
        self.cache_dir
            .clone()
            .or(environment)
            .or_else(|| self.fallback_cache_dir.clone())
    }
...
pub enum BackendEventKind {
    ...
    PlaybackTracksChanged {
        audio: Vec<crate::PlaybackTrack>,
        subtitles: Vec<crate::PlaybackTrack>,
        selected: crate::PlaybackTrackSelection,
    },
...
mod tests {
    ...
    #[test]
    fn cache_directory_preserves_config_environment_and_application_priority() {
        let default_dir = std::path::PathBuf::from("application-default");
        let http_dir = std::path::PathBuf::from("http-override");
        let demux_dir = std::path::PathBuf::from("demux-override");
        let mut config = PlaybackCacheConfig::default();
        assert_eq!(config.resolve_cache_directory(None), None);
        config.fallback_cache_dir = Some(default_dir.clone());
        assert_eq!(config.resolve_cache_directory(None), Some(default_dir));
        for environment in [http_dir, demux_dir] {
            assert_eq!(
                config.resolve_cache_directory(Some(environment.clone())),
                Some(environment.clone())
            );
            let explicit = PlaybackCacheConfig {
                cache_dir: Some("configured".into()),
                ..config.clone()
            };
            assert_eq!(
                explicit.resolve_cache_directory(Some(environment)),
