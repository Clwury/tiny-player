pub(in crate::backend::ffmpeg::avio::cache) use std::collections::VecDeque;

pub(in crate::backend::ffmpeg::avio::cache) use crate::backend::PlaybackCacheConfig;

#[cfg(test)]
use super::HTTP_RING_CACHE_CAPACITY;
pub(in crate::backend::ffmpeg::avio::cache) use super::{
    ...
    HTTP_CACHE_STALL_LOG_INTERVAL, HttpCacheConfig, HttpCacheRangeKind, HttpCacheReadError,
...
impl HttpRingCacheState {
    #[cfg(test)]
    pub(in crate::backend::ffmpeg) fn new(start_offset: u64) -> Self {
    ...
    #[cfg(test)]
    pub(in crate::backend::ffmpeg) fn new_with_cache_capacity(
        start_offset: u64,
        capacity: usize,
    ) -> Self {
    ...
    #[cfg(test)]
    pub(in crate::backend::ffmpeg) fn new_with_readahead_for_test(
        start_offset: u64,
        capacity: usize,
    ...
    #[cfg(test)]
    pub(in crate::backend::ffmpeg) fn new_with_disk_cache_for_test(
        start_offset: u64,
        capacity: usize,
    ...
    pub(in crate::backend::ffmpeg::avio::cache) fn new_with_config(
        start_offset: u64,
    ...
    pub(in crate::backend::ffmpeg::avio::cache) fn apply_prepared_cache_config(
        &mut self,
        cache_config: &PlaybackCacheConfig,
    ) {
        let config = HttpCacheConfig::from_playback_config(cache_config);
    ...
    pub(in crate::backend::ffmpeg) fn with_content_len_hint(
        mut self,
    ...
    #[cfg(test)]
    pub(in crate::backend::ffmpeg) fn set_duration_seconds_for_test(
        &mut self,
