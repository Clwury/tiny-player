...
pub(in crate::backend::ffmpeg::avio::cache) struct PreparedByteAppend {
    pages: VecDeque<BytePage>,
    len: usize,
}

impl PreparedByteAppend {
    pub(in crate::backend::ffmpeg::avio::cache) fn from_bytes(data: &[u8]) -> Self {
    ...
    pub(in crate::backend::ffmpeg::avio::cache) fn len(&self) -> usize {
        self.len
    }

    pub(in crate::backend::ffmpeg::avio::cache) fn is_empty(&self) -> bool {
        self.len == 0
    }

    pub(in crate::backend::ffmpeg::avio::cache) fn discard_front(&mut self, len: usize) {
    ...
    pub(in crate::backend::ffmpeg::avio::cache) fn from_buffer(buffer: ByteRingBuffer) -> Self {
        Self {
            pages: buffer.pages,
            len: buffer.len,
        }
...
pub(in crate::backend::ffmpeg::avio::cache) struct ByteRingBuffer {
    pages: VecDeque<BytePage>,
    len: usize,
    max_capacity: usize,
    page_size: usize,
...
impl ByteRingBuffer {
    pub(in crate::backend::ffmpeg::avio::cache) fn new(max_capacity: usize) -> Self {
    ...
    #[cfg(test)]
    pub(in crate::backend::ffmpeg) fn new_with_page_size_for_test(
        max_capacity: usize,
        page_size: usize,
    ) -> Self {
    ...
    pub(in crate::backend::ffmpeg::avio::cache) fn max_capacity(&self) -> usize {
        self.max_capacity
    }

    pub(in crate::backend::ffmpeg::avio::cache) fn clear(&mut self) {
        self.pages.clear();
        self.len = 0;
    }
    ...
    #[cfg(test)]
    pub(in crate::backend::ffmpeg::avio::cache) fn append(&mut self, data: &[u8]) {
        let prepared = PreparedByteAppend::from_bytes_with_page_size(data, self.page_size);
        self.append_prepared(prepared);
    }

    pub(in crate::backend::ffmpeg::avio::cache) fn append_prepared(
        &mut self,
    ...
    pub(in crate::backend::ffmpeg::avio::cache) fn copy_at(
        &self,
        offset: usize,
        output: &mut [u8],
    ) -> usize {
    ...
    /// same slab.
    pub(in crate::backend::ffmpeg::avio::cache) fn split_off(&mut self, offset: usize) -> Self {
    ...
    pub(in crate::backend::ffmpeg::avio::cache) fn resize_capacity(&mut self, max_capacity: usize) {
    ...
    #[cfg(test)]
    pub(in crate::backend::ffmpeg) fn page_count_for_test(&self) -> usize {
        self.pages.len()
    }

    #[cfg(test)]
    pub(in crate::backend::ffmpeg) fn page_data_ptrs_for_test(&self) -> Vec<usize> {
        self.pages
            .iter()
