...
impl HttpDiskCache {
    pub(in crate::backend::ffmpeg::avio::cache) fn new(
        max_bytes: u64,
    ...
    #[cfg(test)]
    pub(in crate::backend::ffmpeg::avio::cache) fn write_at(
        &mut self,
        offset: u64,
        data: &[u8],
    ) -> std::io::Result<()> {
    ...
    pub(in crate::backend::ffmpeg::avio::cache) fn reserve_write(
        &mut self,
        offset: u64,
        len: usize,
    ...
    pub(in crate::backend::ffmpeg::avio::cache) fn read_at(
        &mut self,
        offset: u64,
        output: &mut [u8],
    ) -> Option<usize> {
    ...
    pub(in crate::backend::ffmpeg::avio::cache) fn add_range(
        &mut self,
        start: u64,
        block: Arc<DiskBlock>,
    ) {
    ...
    pub(in crate::backend::ffmpeg::avio::cache) fn file_bytes(&self) -> u64 {
        self.storage.file_len()
    }

    pub(in crate::backend::ffmpeg::avio::cache) fn set_limit(&mut self, max_bytes: u64) {
    ...
    pub(in crate::backend::ffmpeg::avio::cache) fn next_access_generation(&mut self) -> u64 {
        self.access_generation = self.access_generation.saturating_add(1);
        self.access_generation
    }

    pub(in crate::backend::ffmpeg::avio::cache) fn range_index_containing(
        &self,
        offset: u64,
    ) -> Option<usize> {
        self.ranges
