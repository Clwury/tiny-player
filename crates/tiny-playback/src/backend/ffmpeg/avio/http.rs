...
pub(in crate::backend::ffmpeg) fn input_format_options(
    url: &str,
...
pub(in crate::backend::ffmpeg) fn ffmpeg_http_headers(
...
pub(in crate::backend::ffmpeg) fn should_cache_http_url(url: &str) -> bool {
    let url = url.trim_start().to_ascii_lowercase();
    url.starts_with("http://") || url.starts_with("https://")
}

pub(in crate::backend::ffmpeg) fn reqwest_header_pairs(
    headers: &[(String, String)],
...
#[cfg(test)]
pub(in crate::backend::ffmpeg) fn http_cache_request_headers_for_log(
...
#[cfg(test)]
pub(in crate::backend::ffmpeg) fn http_cache_response_headers_for_log(
...
pub(in crate::backend::ffmpeg) fn http_cache_range_header(
    offset: u64,
...
pub(in crate::backend::ffmpeg) fn http_cache_range_request_len(
    offset: u64,
    content_len: Option<u64>,
    request_bytes: u64,
) -> u64 {
...
pub(in crate::backend::ffmpeg) fn http_cache_playback_range_request_bytes(
    configured_request_bytes: u64,
) -> u64 {
    configured_request_bytes.max(1)
}

pub(in crate::backend::ffmpeg) fn http_cache_range_request_timeout(request_len: u64) -> Duration {
...
pub(in crate::backend::ffmpeg) struct HttpContentRange {
    pub(in crate::backend::ffmpeg) start: u64,
    pub(in crate::backend::ffmpeg) end: u64,
    pub(in crate::backend::ffmpeg) total: Option<u64>,
}

pub(in crate::backend::ffmpeg) fn content_len_from_content_range(
    headers: &reqwest::header::HeaderMap,
) -> Option<u64> {
...
pub(in crate::backend::ffmpeg) fn content_range_from_headers(
    headers: &reqwest::header::HeaderMap,
