...
impl PlaybackOutputScheduler {
    pub(in crate::backend::ffmpeg) fn mark_first_frame_queued(&mut self) {
    ...
    pub(in crate::backend::ffmpeg) fn mark_first_frame_presented(&mut self) -> Option<Duration> {
        if !self.restart_pending() {
            return None;
        }
    ...
    pub(in crate::backend::ffmpeg) fn mark_first_frame_presentation_failed(&mut self) {
    ...
    pub(in crate::backend::ffmpeg) fn initial_start_phase(&self) -> &'static str {
    ...
    pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn initial_av_start_transaction(
        &self,
    ...
    #[cfg(test)]
    pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn begin_initial_av_start_transaction(
        &mut self,
    ...
    pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn begin_initial_av_start_transaction_for_generations(
        &mut self,
        video_anchor_nsecs: u64,
        audio_start_target_nsecs: u64,
        seek_generation: u64,
    ...
    pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn note_initial_av_pair(
        &mut self,
        now: Instant,
    ) {
    ...
    pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn begin_initial_audio_prepare(
        &mut self,
        transaction_id: u64,
        audio_epoch: u64,
    ) -> bool {
    ...
    pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn finish_initial_audio_prepare(
        &mut self,
    ...
    pub(in crate::backend::ffmpeg::playback_loop) fn initial_audio_prepare_phase(
        &self,
    ...
    pub(in crate::backend::ffmpeg::playback_loop) fn initial_audio_prepare_token(
        &self,
    ...
    pub(in crate::backend::ffmpeg::playback_loop) fn initial_audio_prepare_target_nsecs(
        &self,
    ) -> Option<u64> {
        self.initial_av_start_transaction
    ...
    pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn abort_initial_audio_prepare(
        &mut self,
        transaction_id: u64,
    ...
    pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn initial_av_pair_watchdog_expired(
        &self,
        now: Instant,
    ) -> bool {
    ...
    pub(in crate::backend::ffmpeg) fn output_service_demand(
        &self,
        now: Instant,
    ) -> OutputServiceDemand {
        if self
    ...
    pub(in crate::backend::ffmpeg::playback_loop) fn defer_initial_audio_start_retry(
        &mut self,
        now: Instant,
    ...
    pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn preserve_initial_audio_prepare_for_retry(
        &mut self,
        transaction_id: u64,
        now: Instant,
        reason: InitialAudioTransientRetry,
    ...
    pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn wait_initial_audio_start_for_state_change(
        &mut self,
        transaction_id: u64,
    ) -> bool {
        let Some(transaction) = self.initial_av_start_transaction.as_mut() else {
    ...
    pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn observe_initial_sync_log(
        &mut self,
    ...
    pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn observe_initial_audio_defer_log(
        &mut self,
    ...
    pub(in crate::backend::ffmpeg::playback_loop) fn note_output_housekeeping_change(&mut self) {
    ...
    pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn observe_prestart_audio_ownership_log(
        &mut self,
    ...
    pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn observe_output_gate_block_log(
        &mut self,
    ...
    pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn clear_output_gate_block_log(
        &mut self,
    ) {
        self.output_gate_block_log_state = None;
    }

    pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn mark_output_housekeeping_serviced(
        &mut self,
    ) {
        self.mark_output_housekeeping_serviced_at(Instant::now());
    }

    pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn mark_output_housekeeping_serviced_at(
        &mut self,
        now: Instant,
    ) {
    ...
    pub(in crate::backend::ffmpeg) fn output_housekeeping_deadline(&self) -> Option<Instant> {
        let now = Instant::now();
    ...
    pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn fail_initial_av_start_transaction(
        &mut self,
    ...
    pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn fail_initial_av_start_transaction_at_anchor(
        &mut self,
        control: &FfmpegControl,
        session_id: PlaybackSessionId,
        reason: &'static str,
    ...
    #[cfg(test)]
    pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn commit_initial_av_start_transaction(
        &mut self,
    ) {
    ...
    pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn commit_initial_audio_prepare(
        &mut self,
        token: InitialAudioPrepareToken,
    ) -> bool {
        let Some(transaction) = self.initial_av_start_transaction.as_mut() else {
    ...
    pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn finalize_initial_audio_prepare(
        &mut self,
        token: InitialAudioPrepareToken,
        session_id: PlaybackSessionId,
    ) -> bool {
