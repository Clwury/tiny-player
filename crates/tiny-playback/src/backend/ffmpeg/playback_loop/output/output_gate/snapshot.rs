...
impl PlaybackOutputScheduler {
    pub(in crate::backend::ffmpeg) fn snapshot(&self) -> PlaybackOutputSnapshot {
        self.snapshot_for_played_until(None)
    }

    pub(in crate::backend::ffmpeg) fn snapshot_for_played_until(
        &self,
...
impl PlaybackOutputSnapshot {
    pub(in crate::backend::ffmpeg) fn queued_video_bootstrap_forward_nsecs(self) -> u64 {
        self.queued_video_contiguous_forward_nsecs
            .or(self.queued_video_forward_nsecs)
            .unwrap_or(self.queued_video_duration_nsecs)
    }

    pub(in crate::backend::ffmpeg) fn waiting_for_demux(self) -> bool {
        !self.first_video_frame_pending && self.queued_video_frames == 0
    }

    pub(in crate::backend::ffmpeg) fn underflowing(self) -> bool {
        self.waiting_for_demux() || self.video_output_low_water
    }

    pub(in crate::backend::ffmpeg) fn should_wait_for_demux(self) -> bool {
