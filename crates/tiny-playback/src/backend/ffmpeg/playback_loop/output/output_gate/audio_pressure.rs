...
pub(in crate::backend::ffmpeg::playback_loop::output_gate) enum PendingAudioPressureContext {
    StartupSync,
    RebufferResume,
    PlayingSteady,
}
...
impl PendingStartAudioPressureLevel {
    pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn from_duration(
        duration: Duration,
    ) -> Self {
...
pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn audio_output_contiguous_start_timeline_nsecs(
...
pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn playing_pending_audio_limit_duration()
-> Duration {
    AUDIO_OUTPUT_DELAY_LIMIT.saturating_add(AUDIO_OUTPUT_VIDEO_LEAD_DURATION)
}

pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn playing_pending_audio_warn_entry_duration()
-> Duration {
    playing_pending_audio_limit_duration()
        .saturating_add(PLAYING_PENDING_AUDIO_WARN_ENTRY_FRAME_TOLERANCE)
}

pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn playing_pending_audio_pressure_clear_duration()
-> Duration {
    playing_pending_audio_limit_duration().saturating_sub(Duration::from_millis(100))
}

pub(in crate::backend::ffmpeg::playback_loop) enum DecodedAudioAdmission {
    Accepted,
...
pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn audio_output_flush_until_timeline_nsecs(
    snapshot: AudioOutputSnapshot,
...
impl PlaybackOutputScheduler {
    pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn pending_audio_retention_plan(
        &self,
    ...
    pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn bounded_delayed_audio_start_for_retention_plan(
        &self,
    ...
    pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn candidate_bounded_delayed_audio_start_for_retention_plan(
        &self,
        plan: PendingAudioRetentionPlan,
    ) -> Option<u64> {
        if self
    ...
    pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn refresh_initial_bounded_delayed_audio_start_plan(
        &mut self,
    ) -> Option<u64> {
    ...
    pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn unbounded_delayed_audio_start_for_retention_plan(
        &self,
        plan: PendingAudioRetentionPlan,
    ) -> Option<(u64, u64)> {
        if self
    ...
    pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn initial_audio_retention_invariant_satisfied(
        &self,
        plan: PendingAudioRetentionPlan,
    ) -> bool {
    ...
    pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn trim_pending_audio_to_retention_plan(
        &mut self,
        plan: PendingAudioRetentionPlan,
        sample_rate: i32,
        channels: i32,
    ...
    pub(in crate::backend::ffmpeg) fn waiting_for_output_resume(&self) -> bool {
        self.restart_pending() || self.playback_output_state.rebuffering()
    }

    pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn audio_output_steady_target_duration(
        &self,
    ...
    pub(in crate::backend::ffmpeg) fn pending_start_audio_backpressured(&self) -> bool {
    ...
    pub(in crate::backend::ffmpeg) fn output_wait_audio_input_backpressured(&self) -> bool {
    ...
    pub(in crate::backend::ffmpeg) fn pending_resume_audio_limit_reached(&self) -> bool {
    ...
    pub(in crate::backend::ffmpeg) fn pending_audio_contiguous_range_nsecs(
        &self,
    ) -> Option<(u64, u64)> {
        self.pending_start_audio.contiguous_range_nsecs()
    }

    pub(in crate::backend::ffmpeg) fn pending_audio_timeline_gap_near(
        &self,
    ...
    pub(in crate::backend::ffmpeg) fn audio_resume_waterline_for_output_wait(
        &self,
    ...
    pub(in crate::backend::ffmpeg) fn audio_resume_waterline_below_input_suppression(
        &self,
        audio_snapshot: Option<AudioOutputSnapshot>,
        audio_decode_queued_nsecs: u64,
        audio_decode_in_flight_packets: usize,
    ...
    #[allow(clippy::too_many_arguments)]
    pub(in crate::backend::ffmpeg) fn discard_stale_pending_audio_before_output_resume(
        &mut self,
        audio_snapshot: Option<AudioOutputSnapshot>,
        audio_decode_queued_nsecs: u64,
        audio_decode_in_flight_packets: usize,
    ...
    pub(in crate::backend::ffmpeg) fn pending_start_audio_can_recover_output(
        &self,
        audio_snapshot: Option<AudioOutputSnapshot>,
    ) -> bool {
        if self.playback_output_state != PlaybackOutputState::Playing
    ...
    #[allow(clippy::too_many_arguments)]
    pub(in crate::backend::ffmpeg::playback_loop) fn push_decoded_audio_or_buffer(
        &mut self,
        output: &AudioOutput,
    ...
    pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn defer_decoded_audio_for_backpressure(
        &mut self,
        audio: DecodedAudio,
        start_timeline_nsecs: u64,
        end_timeline_nsecs: u64,
    ...
    #[allow(clippy::too_many_arguments)]
    pub(in crate::backend::ffmpeg::playback_loop) fn flush_pending_start_audio_if_ready(
        &mut self,
        output: &AudioOutput,
        control: &FfmpegControl,
        session_id: PlaybackSessionId,
    ...
    pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn decoded_audio_can_push_directly(
        &self,
        start_timeline_nsecs: u64,
        end_timeline_nsecs: u64,
    ...
    pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn defer_next_pending_start_audio_flush(
        &mut self,
    ) {
        self.defer_pending_start_audio_flush_once = true;
    }

    pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn defer_next_pending_start_audio_flush_after_initial_start(
        &mut self,
    ) {
        self.defer_pending_start_audio_flush_once = true;
    ...
    pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn pending_audio_pressure_context(
        &self,
    ...
    pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn report_playing_pending_start_audio_pressure(
        &mut self,
        session_id: PlaybackSessionId,
        reason: &'static str,
    ) {
    ...
    pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn recover_runaway_playing_pending_audio_if_needed(
        &mut self,
        output: &AudioOutput,
        control: &FfmpegControl,
        session_id: PlaybackSessionId,
