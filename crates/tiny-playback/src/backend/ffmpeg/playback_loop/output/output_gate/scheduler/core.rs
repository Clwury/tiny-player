...
impl PlaybackOutputScheduler {
    pub(in crate::backend::ffmpeg) fn record_coordinator_tick(&mut self, elapsed: Duration) {
        self.scheduled_video_queue
            .record_coordinator_tick(elapsed, Instant::now());
    }

    pub(in crate::backend::ffmpeg) fn audio_realign_coverage(
        &self,
    ...
    pub(in crate::backend::ffmpeg) fn new() -> Self {
    ...
    pub(in crate::backend::ffmpeg) fn reset(&mut self, control: &FfmpegControl) {
        self.reset_for_presentation_session(control, None);
    }

    pub(in crate::backend::ffmpeg) fn reset_for_session(
        &mut self,
