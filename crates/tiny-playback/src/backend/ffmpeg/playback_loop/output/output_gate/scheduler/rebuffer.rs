...
impl PlaybackOutputScheduler {
    pub(in crate::backend::ffmpeg::playback_loop) fn disable_audio(
        &mut self,
    ...
    pub(in crate::backend::ffmpeg) fn clear_rebuffer(&mut self, control: &FfmpegControl) {
    ...
    pub(in crate::backend::ffmpeg) fn output_fill_phase(&self) -> bool {
    ...
    pub(in crate::backend::ffmpeg) fn discontinuity_epoch(&self) -> u64 {
        self.discontinuity_epoch
    }

    pub(in crate::backend::ffmpeg) fn restart_fallback_deadline_armed(&self) -> bool {
    ...
    pub(in crate::backend::ffmpeg) fn advance_discontinuity_epoch(&mut self) -> u64 {
        self.discontinuity_epoch = self.discontinuity_epoch.saturating_add(1);
        self.discontinuity_epoch
    }

    pub(in crate::backend::ffmpeg) fn set_state(&mut self, state: PlaybackOutputState) {
    ...
    pub(in crate::backend::ffmpeg) fn start_video_deadline_service(
        &mut self,
    ...
    pub(in crate::backend::ffmpeg) fn update_video_deadline_audio_clock(
        &mut self,
        audio_clock: Option<AudioClockHandle>,
    ) {
        self.video_deadline_audio_clock_available = audio_clock.is_some();
    ...
    pub(in crate::backend::ffmpeg) fn set_video_deadline_audio_clock_available(
        &mut self,
        available: bool,
    ) {
    ...
    pub(in crate::backend::ffmpeg) fn observe_audio_output_activity(
        &mut self,
        now: Instant,
    ...
    pub(in crate::backend::ffmpeg) fn audio_output_clock_stall_fallback_active(&self) -> bool {
        self.audio_output_clock_stall_fallback_active
    }

    pub(in crate::backend::ffmpeg) fn reset_audio_output_activity_watchdog(&mut self) {
        self.audio_output_activity_watchdog = None;
        self.audio_output_clock_stall_fallback_active = false;
    }

    pub(in crate::backend::ffmpeg::playback_loop) fn note_video_output_rebuffer_started(
        &mut self,
        now: Instant,
    ) {
        if self.playback_output_state.rebuffering() {
    ...
    pub(in crate::backend::ffmpeg::playback_loop) fn mark_video_clock_anchor_valid(&mut self) {
        self.video_clock_anchor_valid = true;
        self.output_clock_running = true;
    }

    pub(in crate::backend::ffmpeg::playback_loop) fn video_clock_anchor_valid(&self) -> bool {
        self.video_clock_anchor_valid
    }

    pub(in crate::backend::ffmpeg::playback_loop::output_gate) fn rebuffer_pause_elapsed(
        &self,
    ) -> Option<Duration> {
    ...
    pub(in crate::backend::ffmpeg) fn finish_rebuffer_if_ready(
        &mut self,
    ...
    pub(in crate::backend::ffmpeg) fn begin_video_bootstrap_after_seek(
        &mut self,
        session_id: PlaybackSessionId,
        reason: &'static str,
    ) {
    ...
    pub(in crate::backend::ffmpeg) fn maybe_enter_video_output_rebuffer(
        &mut self,
        now: Instant,
