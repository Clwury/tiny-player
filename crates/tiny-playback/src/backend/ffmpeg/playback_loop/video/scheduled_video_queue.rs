...
pub(in crate::backend::ffmpeg) struct ScheduledVideoQueue {
...
#[derive(Clone)]
pub(in crate::backend::ffmpeg) struct ScheduledVideoDeadlineQueue {
    shared: Arc<ScheduledVideoQueueShared>,
}

pub(in crate::backend::ffmpeg) struct DeadlineVideoFrame {
    pub(in crate::backend::ffmpeg) frame: QueuedVideoFrame,
    generation: u64,
...
pub(in crate::backend::ffmpeg) struct RecentCoordinatorStall {
    pub(in crate::backend::ffmpeg) elapsed: Duration,
    pub(in crate::backend::ffmpeg) age: Duration,
}

#[derive(Default)]
pub(in crate::backend::ffmpeg) struct AudioClockedVideoPopResult {
    pub(in crate::backend::ffmpeg) frame: Option<QueuedVideoFrame>,
    pub(in crate::backend::ffmpeg) dropped_frames: usize,
    pub(in crate::backend::ffmpeg) forced_for_minimum_frame_rate: bool,
}
...
pub(in crate::backend::ffmpeg) const VIDEO_TIMESTAMP_ROUNDING_TOLERANCE_NSECS: u64 = 1_000;
...
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
pub(in crate::backend::ffmpeg) struct QueuedVideoContinuity {
    pub(in crate::backend::ffmpeg) buffered_until_nsecs: Option<u64>,
    pub(in crate::backend::ffmpeg) forward_nsecs: Option<u64>,
    pub(in crate::backend::ffmpeg) largest_gap_nsecs: Option<u64>,
}

pub(in crate::backend::ffmpeg) fn queued_video_continuity_gap_threshold_nsecs(
...
pub(in crate::backend::ffmpeg) fn video_timestamp_gap_within_threshold(
    gap_nsecs: u64,
    max_gap_nsecs: u64,
) -> bool {
...
pub(in crate::backend::ffmpeg) fn discard_queued_video_before(
...
pub(in crate::backend::ffmpeg) fn push_queued_video_frame(
    queued_video_frames: &mut VecDeque<QueuedVideoFrame>,
...
pub(in crate::backend::ffmpeg) fn queued_video_buffered_until_nsecs(
...
pub(in crate::backend::ffmpeg) fn queued_video_buffered_until_from_nsecs(
    queued_video_frames: &VecDeque<QueuedVideoFrame>,
    timeline_nsecs: u64,
) -> Option<u64> {
    let start_index =
...
pub(in crate::backend::ffmpeg) fn queued_video_contiguous_buffered_until_from_nsecs(
    queued_video_frames: &VecDeque<QueuedVideoFrame>,
    timeline_nsecs: u64,
    max_gap_nsecs: u64,
) -> Option<u64> {
...
pub(in crate::backend::ffmpeg) fn queued_video_largest_gap_nsecs(
    queued_video_frames: &VecDeque<QueuedVideoFrame>,
) -> Option<u64> {
...
pub(in crate::backend::ffmpeg) fn queued_video_continuity_from_nsecs(
    queued_video_frames: &VecDeque<QueuedVideoFrame>,
    timeline_nsecs: u64,
...
pub(in crate::backend::ffmpeg) fn queued_video_range_nsecs(
    queued_video_frames: &VecDeque<QueuedVideoFrame>,
) -> Option<(u64, u64)> {
...
pub(in crate::backend::ffmpeg) fn queued_video_forward_nsecs_from(
    queued_video_frames: &VecDeque<QueuedVideoFrame>,
    timeline_nsecs: u64,
) -> Option<u64> {
...
pub(in crate::backend::ffmpeg) fn video_output_rebuffer_low_water(
    queued_video_frames: &VecDeque<QueuedVideoFrame>,
...
pub(in crate::backend::ffmpeg) fn audio_output_video_lead_until_from_nsecs(
    queued_video_frames: &VecDeque<QueuedVideoFrame>,
    timeline_nsecs: u64,
) -> Option<u64> {
    queued_video_buffered_until_from_nsecs(queued_video_frames, timeline_nsecs)
...
impl ScheduledVideoQueue {
    pub(in crate::backend::ffmpeg) fn clear(&mut self) {
        self.clear_for_presentation_session(None);
    }

    pub(in crate::backend::ffmpeg) fn clear_and_bind_presentation_session(
        &mut self,
        session_id: PlaybackSessionId,
    ) {
    ...
    pub(in crate::backend::ffmpeg) fn len(&self) -> usize {
        self.shared.frames().len()
    }

    pub(in crate::backend::ffmpeg) fn is_empty(&self) -> bool {
        self.shared.frames().is_empty()
    }

    pub(in crate::backend::ffmpeg) fn with_frames<R>(
        &self,
    ...
    pub(in crate::backend::ffmpeg) fn contiguous_frame_count_from(
        &self,
        timeline_nsecs: u64,
    ) -> usize {
    ...
    pub(in crate::backend::ffmpeg) fn front_ready_for(
        &self,
    ...
    pub(in crate::backend::ffmpeg) fn pop_front(&mut self) -> Option<QueuedVideoFrame> {
    ...
    pub(in crate::backend::ffmpeg) fn duration_nsecs(&self) -> u64 {
        self.with_frames(|frames| duration_nsecs(queued_video_duration(frames)))
    }

    pub(in crate::backend::ffmpeg) fn duration(&self) -> Duration {
        self.with_frames(queued_video_duration)
    }

    pub(in crate::backend::ffmpeg) fn range_span_nsecs(&self) -> u64 {
        self.with_frames(|frames| duration_nsecs(queued_video_range_span(frames)))
    }

    pub(in crate::backend::ffmpeg) fn range_nsecs(&self) -> Option<(u64, u64)> {
        self.with_frames(queued_video_range_nsecs)
    }

    pub(in crate::backend::ffmpeg) fn back_timing_nsecs(&self) -> Option<(u64, u64)> {
        self.with_frames(|frames| {
            frames
                .back()
    ...
    pub(in crate::backend::ffmpeg) fn extend_back_duration_to(
        &mut self,
        timeline_nsecs: u64,
    ) -> Option<(u64, u64)> {
    ...
    pub(in crate::backend::ffmpeg) fn buffered_until_from_nsecs(
        &self,
        timeline_nsecs: u64,
    ) -> Option<u64> {
    ...
    pub(in crate::backend::ffmpeg) fn forward_nsecs_from(
        &self,
        timeline_nsecs: u64,
    ) -> Option<u64> {
    ...
    pub(in crate::backend::ffmpeg) fn strict_forward_nsecs_from(
        &self,
        timeline_nsecs: u64,
    ) -> Option<u64> {
        self.with_frames(|frames| {
    ...
    pub(in crate::backend::ffmpeg) fn continuity_from_nsecs(
        &self,
        timeline_nsecs: u64,
    ) -> QueuedVideoContinuity {
    ...
    pub(in crate::backend::ffmpeg) fn largest_gap_nsecs(&self) -> Option<u64> {
        self.with_frames(queued_video_largest_gap_nsecs)
    }

    pub(in crate::backend::ffmpeg) fn low_water(&self, played_until_nsecs: u64) -> bool {
        self.with_frames(|frames| video_output_rebuffer_low_water(frames, played_until_nsecs))
    }

    pub(in crate::backend::ffmpeg) fn limit_reached(&self, needs_subtitle_prefetch: bool) -> bool {
        self.with_frames(|frames| queued_video_limit_reached(frames, needs_subtitle_prefetch))
    }

    pub(in crate::backend::ffmpeg) fn limit_frames(&self, needs_subtitle_prefetch: bool) -> usize {
        self.with_frames(|frames| queued_video_limit_frames(frames, needs_subtitle_prefetch))
    }

    pub(in crate::backend::ffmpeg) fn limit_duration(
        &self,
    ...
    pub(in crate::backend::ffmpeg) fn target_frames(&self, needs_subtitle_prefetch: bool) -> usize {
        self.with_frames(|frames| queued_video_target_frames(frames, needs_subtitle_prefetch))
    }

    pub(in crate::backend::ffmpeg) fn target_duration(
        &self,
        needs_subtitle_prefetch: bool,
    ) -> Duration {
    ...
    pub(in crate::backend::ffmpeg) fn skip_nonref_for_pressure(
        &self,
    ...
    pub(in crate::backend::ffmpeg) fn audio_output_lead_until_from_nsecs(
        &self,
        timeline_nsecs: u64,
    ) -> Option<u64> {
    ...
    pub(in crate::backend::ffmpeg) fn audio_clock_wait_duration(
        &self,
        played_until_nsecs: u64,
    ) -> Option<Duration> {
    ...
    pub(in crate::backend::ffmpeg) fn push_queued(&mut self, frame: QueuedVideoFrame) {
        push_queued_video_frame(&mut self.shared.frames(), frame);
        self.shared.changed.notify_all();
    }

    pub(in crate::backend::ffmpeg) fn discard_before(&mut self, timeline_nsecs: u64) -> usize {
        let _admission = self
            .shared
            .admission_gate
            .lock()
    ...
    pub(in crate::backend::ffmpeg) fn discard_at_or_after(&mut self, timeline_nsecs: u64) -> usize {
        let _admission = self
            .shared
            .admission_gate
            .lock()
    ...
    pub(in crate::backend::ffmpeg) fn append_from(&mut self, other: &mut Self) -> usize {
    ...
    pub(in crate::backend::ffmpeg) fn pop_audio_clocked_frame(
        &mut self,
        played_until_nsecs: u64,
    ...
    pub(in crate::backend::ffmpeg) fn scheduler_dropped_video_frames(&self) -> u64 {
        self.shared
            .scheduler_dropped_video_frames
            .load(Ordering::Relaxed)
    }

    pub(in crate::backend::ffmpeg) fn attach_deadline_service(
        &self,
    ...
    pub(in crate::backend::ffmpeg) fn set_deadline_service_active(&self, active: bool) {
        let _admission = self
            .shared
            .admission_gate
            .lock()
    ...
    pub(in crate::backend::ffmpeg) fn deadline_service_owns_presentation(&self) -> bool {
        self.shared
            .deadline_service_attached
    ...
    pub(in crate::backend::ffmpeg) fn prebuffer_limit_log_summary(
        &mut self,
        now: Instant,
    ) -> Option<u64> {
        if self
    ...
    pub(in crate::backend::ffmpeg) fn record_coordinator_tick(
        &mut self,
        elapsed: Duration,
        completed_at: Instant,
    ) {
    ...
    pub(in crate::backend::ffmpeg) fn recent_coordinator_stall(
        &self,
        now: Instant,
    ...
    #[allow(clippy::too_many_arguments)]
    pub(in crate::backend::ffmpeg) fn log_resume_waterline_wait(
        &self,
        session_id: PlaybackSessionId,
        context: &'static str,
...
impl ScheduledVideoDeadlineQueue {
    pub(in crate::backend::ffmpeg) fn presentation_session_id(&self) -> PlaybackSessionId {
        PlaybackSessionId(self.shared.presentation_session_id.load(Ordering::Acquire))
    }

    pub(in crate::backend::ffmpeg) fn active(&self) -> bool {
        self.shared.deadline_service_active.load(Ordering::Acquire)
    }

    pub(in crate::backend::ffmpeg) fn suspend_for_session_mismatch(
        &self,
    ...
    pub(in crate::backend::ffmpeg) fn wait_for_change(&self, timeout: Duration) {
    ...
    pub(in crate::backend::ffmpeg) fn wake(&self) {
        self.shared.changed.notify_all();
    }
    ...
    pub(in crate::backend::ffmpeg) fn admit_if_current<R>(
        &self,
    ...
    pub(in crate::backend::ffmpeg) fn detach(&self) {
        let _admission = self
            .shared
            .admission_gate
            .lock()
...
#[cfg(test)]
pub(in crate::backend::ffmpeg) fn pop_audio_clocked_video_frame(
    queued_video_frames: &mut VecDeque<QueuedVideoFrame>,
    played_until_nsecs: u64,
...
#[cfg(test)]
pub(in crate::backend::ffmpeg) fn pop_audio_clocked_video_frame_with_policy(
    queued_video_frames: &mut VecDeque<QueuedVideoFrame>,
    played_until_nsecs: u64,
) -> AudioClockedVideoPopResult {
...
#[cfg(test)]
pub(in crate::backend::ffmpeg) fn pop_audio_clocked_video_frame_with_minimum_rate(
    queued_video_frames: &mut VecDeque<QueuedVideoFrame>,
    played_until_nsecs: u64,
) -> AudioClockedVideoPopResult {
...
pub(in crate::backend::ffmpeg) fn queued_video_frame_ready_for_audio_clock(
    queued_video_frames: &VecDeque<QueuedVideoFrame>,
    played_until_nsecs: u64,
) -> bool {
...
pub(in crate::backend::ffmpeg) fn audio_clocked_video_wait_duration(
    queued_video_frames: &VecDeque<QueuedVideoFrame>,
    played_until_nsecs: u64,
) -> Option<Duration> {
...
pub(in crate::backend::ffmpeg) fn should_drop_late_video_frame(
