...
pub(in crate::backend::ffmpeg) enum AudioClockedVideoDrainStatus {
    Drained,
    WaitingAudio { made_progress: bool },
    Interrupted,
}

pub(in crate::backend::ffmpeg) fn report_first_video_frame_presented(
...
impl AudioClockedVideoDrainStatus {
    pub(in crate::backend::ffmpeg) fn made_progress(self) -> bool {
        matches!(
            self,
            Self::WaitingAudio {
                made_progress: true
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(in crate::backend::ffmpeg) struct AudioClockAvailability {
    pub(in crate::backend::ffmpeg) has_audio_output: bool,
    pub(in crate::backend::ffmpeg) available: bool,
    pub(in crate::backend::ffmpeg) pending_nsecs: Option<u64>,
    pub(in crate::backend::ffmpeg) buffered_until_timeline_nsecs: Option<u64>,
    pub(in crate::backend::ffmpeg) underrun_active: bool,
    pub(in crate::backend::ffmpeg) watchdog_stalled: bool,
}

pub(in crate::backend::ffmpeg) fn audio_clock_availability(
    ...
    Ok(AudioClockAvailability {
        ...
        available: pending_nsecs > 0 && !underrun_active && !watchdog_stalled,
...
pub(in crate::backend::ffmpeg) fn admit_decoded_video_frame_to_vo(
    frame: DecodedFrame,
    session_id: PlaybackSessionId,
    timeline_nsecs: u64,
...
#[allow(clippy::too_many_arguments)]
pub(in crate::backend::ffmpeg) fn service_audio_clocked_video_queue_if_playing(
    output: Option<&AudioOutput>,
...
#[allow(clippy::too_many_arguments)]
pub(in crate::backend::ffmpeg) fn service_decode_backpressure_step(
...
#[allow(clippy::too_many_arguments)]
pub(in crate::backend::ffmpeg) fn service_audio_clocked_video_drain_step(
    output: &AudioOutput,
    control: &FfmpegControl,
    output_scheduler: &mut PlaybackOutputScheduler,
    session_id: PlaybackSessionId,
...
#[allow(clippy::too_many_arguments)]
pub(in crate::backend::ffmpeg) fn service_video_clocked_video_queue(
    scheduler: &PlaybackScheduler,
    control: &FfmpegControl,
    output_scheduler: &mut PlaybackOutputScheduler,
    session_id: PlaybackSessionId,
...
#[allow(clippy::too_many_arguments)]
pub(in crate::backend::ffmpeg) fn service_video_clocked_video_queue_if_audio_clock_unavailable(
    scheduler: &PlaybackScheduler,
    control: &FfmpegControl,
    output_scheduler: &mut PlaybackOutputScheduler,
    audio_clock: AudioClockAvailability,
    ...
    output_scheduler.set_video_deadline_audio_clock_available(audio_clock.available);
    if output_scheduler.playback_output_state.restart_pending() {
        return false;
    }
    if !output_scheduler.video_clock_anchor_valid() {
        tracing::trace!(
            session_id = ?session_id,
            ...
            "holding FFmpeg video-clock drain until scheduler anchor is valid"
        );
        return false;
    }
    ...
    if audio_clock.has_audio_output {
        tracing::debug!(
            ...
            audio_output_watchdog_stalled = audio_clock.watchdog_stalled,
            queued_video_frames = output_scheduler.scheduled_video_queue.len(),
            queued_video_ms =
                output_scheduler.scheduled_video_queue.duration_nsecs() as f64 / 1_000_000.0,
            queued_video_range = ?output_scheduler.scheduled_video_queue.range_nsecs(),
...
fn video_clock_drain_allowed_while_audio_unavailable(audio_clock: AudioClockAvailability) -> bool {
    audio_clock.watchdog_stalled || !audio_clock.has_audio_output || !audio_clock.underrun_active
}

#[allow(clippy::too_many_arguments)]
pub(in crate::backend::ffmpeg) fn present_video_frame_to_vo(
    frame: DecodedFrame,
    timeline_nsecs: u64,
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(in crate::backend::ffmpeg) enum DecodedVideoAdmissionStatus {
    Continue,
    Stop,
}

#[allow(clippy::too_many_arguments)]
pub(in crate::backend::ffmpeg) fn queue_decoded_video_frame(
    output_scheduler: &mut PlaybackOutputScheduler,
    frame: DecodedFrame,
    timeline_nsecs: u64,
    duration_nsecs: u64,
...
#[allow(clippy::too_many_arguments)]
pub(in crate::backend::ffmpeg) fn service_audio_clocked_drain_decoded_video_frame(
    output_scheduler: &mut PlaybackOutputScheduler,
    _scheduler: &mut PlaybackScheduler,
    output: &AudioOutput,
    control: &FfmpegControl,
    session_id: PlaybackSessionId,
    vo_queue: &VideoOutputQueue,
...
#[allow(clippy::too_many_arguments)]
pub(in crate::backend::ffmpeg) fn service_video_clocked_decoded_video_frame(
    scheduler: &mut PlaybackScheduler,
    control: &FfmpegControl,
    output_scheduler: &mut PlaybackOutputScheduler,
    session_id: PlaybackSessionId,
    ...
    if output_scheduler.restart_pending() {
        ...
        tracing::debug!(
            ...
            "presented first video frame for FFmpeg output generation"
        );
    ...
    fn audio_underrun_holds_video_clocked_queue() {
        ...
        assert!(video_clock_drain_allowed_while_audio_unavailable(
            AudioClockAvailability {
                has_audio_output: true,
                available: false,
                pending_nsecs: Some(92_879_818),
                buffered_until_timeline_nsecs: Some(846_325_879_818),
                underrun_active: false,
