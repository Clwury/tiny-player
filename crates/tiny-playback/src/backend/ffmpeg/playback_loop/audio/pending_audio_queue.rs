...
#[derive(Default)]
pub(in crate::backend::ffmpeg) struct PendingStartAudio {
...
pub(in crate::backend::ffmpeg) struct PendingStartAudioFrame {
...
pub(in crate::backend::ffmpeg) struct PendingAudioStageCoverage {
    pub(in crate::backend::ffmpeg) until_nsecs: u64,
    pub(in crate::backend::ffmpeg) payload_nsecs: u64,
}

impl PendingStartAudioFrame {
    pub(in crate::backend::ffmpeg) fn trim_before(
        &mut self,
        timeline_nsecs: u64,
        _sample_rate: c_int,
        channels: c_int,
...
impl PendingStartAudio {
    pub(in crate::backend::ffmpeg) fn push(
        &mut self,
        audio: DecodedAudio,
    ...
    pub(in crate::backend::ffmpeg) fn first_start_timeline_nsecs(&self) -> Option<u64> {
        self.frames.front().map(|frame| frame.start_timeline_nsecs)
    }

    pub(in crate::backend::ffmpeg) fn contiguous_range_nsecs(&self) -> Option<(u64, u64)> {
    ...
    pub(in crate::backend::ffmpeg) fn first_gap_nsecs(&self) -> Option<(u64, u64)> {
    ...
    pub(in crate::backend::ffmpeg) fn contiguous_duration(&self) -> Duration {
        self.contiguous_range_nsecs()
            .map(|(start, end)| Duration::from_nanos(end.saturating_sub(start)))
            .unwrap_or_default()
    }

    pub(in crate::backend::ffmpeg) fn first_start_at_or_after(
        &self,
        timeline_nsecs: u64,
    ) -> Option<u64> {
        self.frames
    ...
    pub(in crate::backend::ffmpeg) fn timeline_gap_near(
        &self,
    ...
    pub(in crate::backend::ffmpeg) fn buffered_until_from(
        &self,
        timeline_nsecs: u64,
    ) -> Option<u64> {
    ...
    pub(in crate::backend::ffmpeg) fn forward_duration_from(
        &self,
        timeline_nsecs: u64,
    ) -> Option<u64> {
    ...
    pub(in crate::backend::ffmpeg) fn stageable_coverage_between(
        &self,
        start_nsecs: u64,
        limit_nsecs: u64,
    ...
    pub(in crate::backend::ffmpeg) fn pop_front_until(
        &mut self,
        end_timeline_nsecs: u64,
    ...
    pub(in crate::backend::ffmpeg) fn push_front_frame(&mut self, frame: PendingStartAudioFrame) {
    ...
    pub(in crate::backend::ffmpeg) fn restore_staged_frames(
        &mut self,
    ...
    pub(in crate::backend::ffmpeg) fn buffered_duration(&self) -> Duration {
        Duration::from_nanos(self.buffered_duration_nsecs)
    }

    #[cfg(test)]
    pub(in crate::backend::ffmpeg) fn queued_samples(&self) -> usize {
        self.buffered_samples
    }
}

pub(in crate::backend::ffmpeg::playback_loop) fn matching_audio_timeline_gap<I>(
    initial_previous_end_nsecs: Option<u64>,
    frames: I,
    expected_previous_end_nsecs: u64,
    expected_next_start_nsecs: u64,
