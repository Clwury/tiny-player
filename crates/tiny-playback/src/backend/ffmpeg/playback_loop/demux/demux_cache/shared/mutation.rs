...
impl DemuxPacketCacheShared {
    pub(in crate::backend::ffmpeg::playback_loop::demux_cache) fn note_producer_recovering(
        &self,
        error: String,
    ...
    pub(in crate::backend::ffmpeg::playback_loop::demux_cache) fn clear_producer_recovery(&self) {
        let mut guard = self
            .state
            .lock()
            .expect("FFmpeg demux packet cache poisoned");
    ...
    pub(in crate::backend::ffmpeg::playback_loop::demux_cache) fn append_packet(
        &self,
    ...
    pub(in crate::backend::ffmpeg::playback_loop::demux_cache) fn mark_eof(&self) {
        let emit = {
            let mut guard = self
                .state
                .lock()
    ...
    pub(in crate::backend::ffmpeg::playback_loop::demux_cache) fn set_error(&self, error: String) {
        let emit = {
            let mut guard = self
                .state
                .lock()
    ...
    pub(in crate::backend::ffmpeg::playback_loop::demux_cache) fn clear_cache_pause_for_decoded_resume(
        &self,
    ) {
        let emit = {
            let mut guard = self
