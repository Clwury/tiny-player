...
use crate::backend::ffmpeg::disk_cache::{BoundedDiskFile, DiskBlock, read_at};

pub(in crate::backend::ffmpeg::playback_loop::demux_cache) struct DemuxPacketDiskCache {
    pub(in crate::backend::ffmpeg::playback_loop::demux_cache) path: PathBuf,
    storage: BoundedDiskFile,
    unlink_on_drop: bool,
}

impl DemuxPacketDiskCache {
    pub(in crate::backend::ffmpeg::playback_loop::demux_cache) fn from_config(
        ...
        Self::new(
            max_bytes,
            config.cache_directory("TINY_DEMUX_PACKET_CACHE_DIR"),
            config.unlink_files,
        )
    }
    ...
    #[cfg(test)]
    pub(in crate::backend::ffmpeg::playback_loop::demux_cache) fn write_packet(
        &mut self,
        data: &[u8],
    ...
    pub(in crate::backend::ffmpeg::playback_loop::demux_cache) fn reserve_packet(
        &self,
        len: usize,
    ...
    pub(in crate::backend::ffmpeg::playback_loop::demux_cache) fn file_bytes(&self) -> u64 {
        self.storage.file_len()
    }

    pub(in crate::backend::ffmpeg::playback_loop::demux_cache) fn limit(&self) -> u64 {
        self.storage.limit()
    }

    pub(in crate::backend::ffmpeg::playback_loop::demux_cache) fn set_limit(
        &self,
        config: &PlaybackCacheConfig,
    ) {
        let budget = config.effective_disk_cache_budgets().1;
    ...
    pub(in crate::backend::ffmpeg::playback_loop::demux_cache) fn maintain_file_size(&self) {
        self.storage.maintain_file_size();
    }

    pub(in crate::backend::ffmpeg::playback_loop::demux_cache) fn accepts(
        &self,
        block: &DiskBlock,
    ) -> bool {
    ...
    #[cfg(test)]
    pub(in crate::backend::ffmpeg::playback_loop::demux_cache) fn read_packet(
        &self,
        block: Arc<DiskBlock>,
        len: usize,
        props: &AvPacket,
...
pub(in crate::backend::ffmpeg::playback_loop::demux_cache) fn read_demux_packet_disk_payload(
    file: &File,
    offset: u64,
    len: usize,
...
pub(in crate::backend::ffmpeg::playback_loop::demux_cache) fn demux_packet_disk_cache_enabled()
-> bool {
