...
impl AudioOutput {
    pub(in crate::backend::ffmpeg) fn clock_handle(&self) -> super::AudioClockHandle {
    ...
    #[cfg(test)]
    pub(in crate::backend::ffmpeg) fn stopped_for_test(
        control: Arc<FfmpegControl>,
        max_samples: usize,
        sample_rate: c_int,
        channels: c_int,
    ...
    pub(in crate::backend::ffmpeg) fn sample_rate(&self) -> c_int {
        self.sample_rate
    }

    pub(in crate::backend::ffmpeg) fn channels(&self) -> c_int {
        self.channels
    }

    pub(in crate::backend::ffmpeg) fn sample_format(&self) -> &str {
        &self.sample_format
    }

    pub(in crate::backend::ffmpeg) fn device_name(&self) -> &str {
        &self.device_name
    }

    pub(in crate::backend::ffmpeg) fn misaligned_audio_buffer_count(&self) -> u64 {
        self.shared
            .misaligned_audio_buffer_count
            .load(Ordering::Relaxed)
    }

    pub(in crate::backend::ffmpeg) fn try_push_timed(
        &self,
        samples: Vec<f32>,
    ...
    pub(in crate::backend::ffmpeg) fn try_push_timed_for_epoch(
        &self,
        mut samples: Vec<f32>,
        start_timeline_nsecs: u64,
        end_timeline_nsecs: u64,
    ...
    pub(in crate::backend::ffmpeg) fn fence_clock_without_wait(&self, timeline_nsecs: u64) -> u64 {
        self.timeline.set_active(false);
    ...
    pub(in crate::backend::ffmpeg) fn audio_epoch(&self) -> u64 {
        self.timeline.epoch()
    }

    pub(in crate::backend::ffmpeg) fn control_seek_generation(&self) -> u64 {
        self.shared.control.seek_generation()
    }

    pub(in crate::backend::ffmpeg) fn deactivate(&self) {
        if let Err(error) = self.pause_stream() {
    ...
    pub(in crate::backend::ffmpeg) fn activate_audio_output(
        &self,
        expected_epoch: u64,
    ...
    pub(in crate::backend::ffmpeg) fn commit_audio_output_control(
        &self,
        expected_epoch: u64,
        expected_seek_generation: u64,
        control: &FfmpegControl,
    ...
    pub(in crate::backend::ffmpeg) fn play_committed_audio_output(
        &self,
    ) -> std::result::Result<(), String> {
        self.play_stream()
    }

    pub(in crate::backend::ffmpeg) fn stream_active(&self) -> bool {
    ...
    #[cfg(test)]
    pub(in crate::backend::ffmpeg) fn stream_active_for_test(&self) -> bool {
        self.stream_active()
    }

    #[cfg(test)]
    pub(in crate::backend::ffmpeg) fn stream_control_counts_for_test(&self) -> (u64, u64) {
        (
    ...
    pub(in crate::backend::ffmpeg) fn activate_current_audio_output(
        &self,
        control: &FfmpegControl,
    ) -> bool {
    ...
    pub(in crate::backend::ffmpeg) fn try_abort_staged_audio(
        &self,
        expected_epoch: u64,
    ...
    pub(in crate::backend::ffmpeg) fn underrun_active(&self) -> bool {
        self.shared.underrun_active.load(Ordering::Acquire)
    }

    pub(in crate::backend::ffmpeg) fn hold_for_underrun_prefill(&self, timeline_nsecs: u64) {
        self.shared.mark_underrun(timeline_nsecs);
    }

    pub(in crate::backend::ffmpeg) fn hold_for_initial_prefill(
        &self,
        timeline_nsecs: u64,
        prefill_nsecs: u64,
    ) {
    ...
    #[cfg(test)]
    pub(in crate::backend::ffmpeg) fn mark_underrun_for_test(&self, timeline_nsecs: u64) {
        self.shared.mark_underrun(timeline_nsecs);
    }

    pub(in crate::backend::ffmpeg) fn drain_deadline(
        &self,
    ...
    pub(in crate::backend::ffmpeg) fn drain_step(
        &self,
        deadline: Instant,
        control: &FfmpegControl,
    ...
    pub(in crate::backend::ffmpeg) fn snapshot(
        &self,
    ...
    pub(in crate::backend::ffmpeg) fn try_snapshot(
        &self,
    ...
    pub(in crate::backend::ffmpeg) fn stable_snapshot(
        &self,
    ...
    pub(in crate::backend::ffmpeg) fn prepared_snapshot(
        &self,
    ) -> std::result::Result<AudioOutputStableSnapshot, String> {
    ...
    pub(in crate::backend::ffmpeg) fn begin_service_stage(
        &self,
    ...
    #[cfg(test)]
    pub(in crate::backend::ffmpeg) fn service_stage_snapshots_for_test(
        &self,
    ...
    #[cfg(test)]
    pub(in crate::backend::ffmpeg) fn hold_internal_locks_until_for_test(
        &self,
    ...
    #[cfg(test)]
    pub(in crate::backend::ffmpeg) fn reset_would_block_for_test(&self) -> bool {
    ...
    #[cfg(test)]
    pub(in crate::backend::ffmpeg) fn invoke_callback_for_test(&self, samples: &mut [f32]) {
        fill_audio_output(samples, &self.shared);
    }

    #[cfg(test)]
    pub(in crate::backend::ffmpeg) fn stage_filtered_audio_for_test(
        &self,
        samples: Vec<f32>,
        start_timeline_nsecs: u64,
        end_timeline_nsecs: u64,
    ...
    #[cfg(test)]
    pub(in crate::backend::ffmpeg) fn transfer_next_queued_frame_for_test(
        &self,
    ...
    pub(in crate::backend::ffmpeg) fn activity_snapshot(
        &self,
...
#[cfg(test)]
pub(in crate::backend::ffmpeg::audio) fn stable_audio_output_snapshot_with_hook_for_test(
    shared: &AudioShared,
    queue: &AudioQueueShared,
    timeline: &AudioTimelineState,
...
#[cfg(test)]
pub(in crate::backend::ffmpeg::audio) fn stable_audio_output_snapshot_with_compose_hook_for_test(
    shared: &AudioShared,
    queue: &AudioQueueShared,
    timeline: &AudioTimelineState,
...
#[cfg(test)]
pub(in crate::backend::ffmpeg::audio) fn stable_audio_output_snapshot_with_retry_hook_for_test(
    shared: &AudioShared,
    queue: &AudioQueueShared,
    timeline: &AudioTimelineState,
