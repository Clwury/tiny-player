...
impl AudioBuffer {
    #[cfg(test)]
    pub(in crate::backend::ffmpeg) fn with_capacity(max_samples: usize) -> Self {
        Self::with_capacity_and_epoch(max_samples, 0)
    }

    pub(in crate::backend::ffmpeg::audio) fn with_capacity_and_epoch(
        max_samples: usize,
        epoch: u64,
    ) -> Self {
        Self {
    ...
    pub(in crate::backend::ffmpeg) fn available_capacity(&self) -> usize {
    ...
    pub(in crate::backend::ffmpeg) fn push_slice(&mut self, samples: &[f32]) -> usize {
    ...
    pub(in crate::backend::ffmpeg) fn pop_sample(&mut self) -> Option<f32> {
...
impl AudioQueueState {
    pub(in crate::backend::ffmpeg::audio) fn new() -> Self {
        Self {
            items: VecDeque::new(),
    ...
    pub(in crate::backend::ffmpeg::audio) fn can_accept(
        &self,
    ...
    pub(in crate::backend::ffmpeg::audio) fn push(&mut self, item: AudioQueueItem) {
    ...
    pub(in crate::backend::ffmpeg::audio) fn finish_item(
        &mut self,
        samples: usize,
        duration_nsecs: u64,
    ) {
    ...
    pub(in crate::backend::ffmpeg::audio) fn clear(&mut self) {
        self.items.clear();
        self.filtered.clear();
        self.tempo = None;
    ...
    pub(in crate::backend::ffmpeg::audio) fn pending_duration(&self) -> Duration {
        Duration::from_nanos(
...
impl AudioQueueShared {
    #[cfg(test)]
    pub(in crate::backend::ffmpeg::audio) fn new(control: Arc<FfmpegControl>) -> Self {
    ...
    pub(in crate::backend::ffmpeg::audio) fn generation(&self) -> u64 {
        self.timeline.epoch()
    }

    pub(in crate::backend::ffmpeg::audio) fn is_current_generation(&self, generation: u64) -> bool {
    ...
    pub(in crate::backend::ffmpeg::audio) fn clear_current_epoch(&self) {
    ...
    pub(in crate::backend::ffmpeg::audio) fn shutdown(&self) {
        self.shutdown.store(true, Ordering::Release);
        self.ready.notify_all();
    }

    pub(in crate::backend::ffmpeg::audio) fn pop(
        &self,
        sample_rate: i32,
        channels: i32,
    ...
    #[cfg(test)]
    pub(in crate::backend::ffmpeg::audio) fn finish_item_with_lock_checkpoint_for_test(
        &self,
        generation: u64,
        samples: usize,
        duration_nsecs: u64,
...
pub(in crate::backend::ffmpeg::audio) fn spawn_audio_queue_worker(
...
pub(in crate::backend::ffmpeg::audio) fn write_audio_queue_item(
    shared: &AudioShared,
    queue: &AudioQueueShared,
    item: AudioQueueItem,
