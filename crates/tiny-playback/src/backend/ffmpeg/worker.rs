...
use crate::render_host::{PlaybackSessionId, VideoOutputQueue};
...
impl FfmpegControl {
    ...
    pub(super) fn set_playback_rate(&self, rate: f64) {
        self.playback_rate.store(
            crate::rate::clamp_playback_rate(rate).to_bits(),
            Ordering::Release,
        );
    }
...
pub(super) struct FfmpegPlaybackInput {
    ...
    pub(super) selected_tracks: crate::PlaybackTrackSelection,
...
fn apply_playback_command(
    ...
    match command {
        ...
        FfmpegCommand::SetPlaybackRate { rate } => {
            drained.playback_rate = Some(crate::rate::clamp_playback_rate(rate));
        }
    }
}
